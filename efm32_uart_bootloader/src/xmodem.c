 ...
 *****************************************************************************/
int XMODEM_download(uint8_t uartID, uint32_t baseAddress, uint32_t endAddress)
{
  XMODEM_packet *pkt;
  uint32_t      i;
  uint32_t      addr;
  ...
  while (1)
  {
    USART_txByte(uartID, XMODEM_NCG);
    for (i = 0; i < 10000000; i++)
    {
      if((uartID == 0 && (BOOTLOADER_USART_A->STATUS & BOOTLOADER_USART_A_STATUS_RXDATAV))
#ifdef BOOTLOADER_USART_B_ID 
         || (uartID == 1 && (BOOTLOADER_USART_B->STATUS & BOOTLOADER_USART_B_STATUS_RXDATAV)) 
#endif
           )
      {
 ...
 xmodem_transfer:
  while (1)
  {
     ...
     * rest of the packet */
    pkt->header = USART_rxByte(uartID);

    /* Check for end of transfer */
    if (pkt->header == XMODEM_EOT)
    {
      /* Acknowledget End of transfer */
      USART_txByte(uartID, XMODEM_ACK);
      break;
    }
...
int XMODEM_downloadAndReset(uint8_t uartID, uint32_t baseAddress, uint32_t endAddress)
{
  XMODEM_download(uartID, baseAddress, endAddress);
  USART_printString(uartID, "Reseting");
