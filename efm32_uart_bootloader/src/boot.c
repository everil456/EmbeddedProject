...
#else
__noreturn void BOOT_jump(uint32_t sp, uint32_t pc)
{
  (void) sp;
  (void) pc;
  /* Set new MSP, PSP based on SP (r0)*/
 ...
 *****************************************************************************/
__noreturn void BOOT_boot(void)
{
  uint32_t pc, sp;
  ...
  /* Reset GPIO settings */
  GPIO->P[BOOTLOADER_USART_A_PORT_TX].MODEL = _GPIO_P_MODEL_RESETVALUE;
#ifdef BOOTLOADER_USART_B_ID 
  GPIO->P[BOOTLOADER_USART_B_PORT_TX].MODEL = _GPIO_P_MODEL_RESETVALUE;
#endif
  /* Disable RTC clock */
